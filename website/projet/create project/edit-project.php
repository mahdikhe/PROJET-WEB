...
// Correct PDO version for SELECT query
...
// Correct PDO version for UPDATE query
...
$stmt->bindParam(':projectLocation', $projectLocation);
$stmt->bindParam(':projectCategory', $projectCategory);
$stmt->bindParam(':projectTags', $projectTags);
$stmt->bindParam(':projectBudget', $projectBudget);
$stmt->bindParam(':fundingGoal', $fundingGoal);
$stmt->bindParam(':teamSize', $teamSize);
$stmt->bindParam(':skillsNeeded', $skillsNeeded);
$stmt->bindParam(':projectVisibility', $projectVisibility);
$stmt->bindParam(':projectImage', $imageDestination);
$stmt->bindParam(':projectWebsite', $projectWebsite);
$stmt->bindParam(':id', $projectId, PDO::PARAM_INT);
...
if ($stmt->execute()) {
    ...
    $error = "Error updating project: " . implode(" ", $stmt->errorInfo());
}
}
?>
<!DOCTYPE html>
