...
<body>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Initialize map
        const map = L.map('contributor-map').setView([48.8566, 2.3522], 5);
    ...
    function displayContributorsOnMap(map, contributors) {
        ...
        contributors.forEach(contributor => {
            if (contributor.latitude && contributor.longitude) {
                ...
                // Add marker to map
                const marker = L.marker([contributor.latitude, contributor.longitude], {
                    icon: customIcon
                }).addTo(map);
                ...
                marker.bindPopup(`
                    ...
                    <span style="color: #666;">${contributor.city || 'Location not specified'}</span><br>
                    <span style="font-size: 0.9em; margin-top: 5px; display: block;">
                        ${contributor.contribution_type || 'General contributor'}
                    </span>
                    ${contributor.project_name ? '<span style="font-size: 0.8em; color: #666;">Project: ' + contributor.project_name + '</span>' : ''}
                `);
                
                bounds.extend([contributor.latitude, contributor.longitude]);
            }
        });
    ...
    function displayContributorsList(contributors) {
        const listContainer = document.getElementById('contributor-list');
        listContainer.innerHTML = '';
        
        if (!contributors || contributors.length === 0) {
            // Add demo data if no contributors
            const demoContributors = getDemoContributors();
            renderContributorsList(demoContributors, listContainer);
            return;
        }
        
        renderContributorsList(contributors, listContainer);
    }
    
    function renderContributorsList(contributors, container) {
        contributors.forEach(contributor => {
            // Determine contributor type CSS class
            let typeClass = 'type-designer';
            let contributionType = contributor.contribution_type || '';
            
            if (/developer|develop/i.test(contributionType)) {
                typeClass = 'type-developer';
            } else if (/manager|lead/i.test(contributionType)) {
                typeClass = 'type-manager';
            } else if (/architect/i.test(contributionType)) {
                typeClass = 'type-architect';
            } else if (/planner|planning/i.test(contributionType)) {
                typeClass = 'type-planner';
            }
            
            // Get initials for avatar
            const firstInitial = contributor.first_name ? contributor.first_name.charAt(0) : '';
            const lastInitial = contributor.last_name ? contributor.last_name.charAt(0) : '';
            ...
            card.innerHTML = `
                ...
                <div class="contributor-info">
                    <div class="contributor-name">${contributor.first_name || ''} ${contributor.last_name || ''}</div>
                    <div class="contributor-meta">
                        <span><i class="fas fa-map-marker-alt"></i> ${contributor.city || 'Unknown Location'}</span>
                        ${contributor.project_name ? `<span><i class="fas fa-project-diagram"></i> ${contributor.project_name}</span>` : ''}
                    </div>
                </div>
                <span class="contributor-type ${typeClass}">${contributor.contribution_type || 'Contributor'}</span>
            `;
    ...
    function getMarkerColor(contributionType) {
        if (!contributionType) return '#6c63ff'; // Default color
        
        contributionType = contributionType.toLowerCase();
        
        if (contributionType.includes('developer') || contributionType.includes('develop')) {
            return '#00b8a9';
        } else if (contributionType.includes('manager') || contributionType.includes('lead')) {
            return '#ff6b6b';
        } else if (contributionType.includes('architect')) {
            return '#ffd166';
        } else if (contributionType.includes('planner') || contributionType.includes('planning')) {
            return '#4CAF50';
        } else {
            return '#6c63ff'; // Default for designer or unknown
        }
    }
    
    function addDemoContributors(map) {
        const demoContributors = getDemoContributors();
        
        // Display on map
        displayContributorsOnMap(map, demoContributors);
    ...
    function getDemoContributors() {
        return [
            {
                first_name: 'Marie', 
                last_name: 'Dubois', 
                city: 'Paris', 
                contribution_type: 'Designer',
                project_name: 'Urban Garden Project',
                latitude: 48.8566, 
                longitude: 2.3522
            },
            {
            ...
            },
            {
                ...
                last_name: 'Leroy', 
                city: 'Marseille', 
                contribution_type: 'Project Manager',
                project_name: 'Urban Garden Project',
                latitude: 43.2965, 
                longitude: 5.3698
            },
            {
                ...
                last_name: 'Benyahia', 
                city: 'Tunis', 
                contribution_type: 'Architect',
                project_name: 'Historical Restoration',
                latitude: 36.8065, 
                longitude: 10.1815
            },
            {
                ...
                last_name: 'Rodriguez', 
                city: 'Barcelona', 
                contribution_type: 'Urban Planner',
                project_name: 'Smart Traffic System',
                latitude: 41.3851, 
                longitude: 2.1734
            },
